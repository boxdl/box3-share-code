 ...
 *
 */
if(window.DUNGEON_HELPER_INSTALLED) {throw new Error("不能重复执行脚本")}
...
function main() {
  ...
  if (!dialog) return setTimeout(main, 10);
...
Object.defineProperty(window,"mergeProblemsLib",{
  ...
  value:(json)=>{
    ...
    console.log(`%c题库新增${localStorage.length-oldLength}条数据`,"color:#34A853")}catch (e) {
...
Object.defineProperty(window,"clearProblemsLib",{
  configurable:true,
  value:(b)=>{
    if(!b){console.warn("此操作会完全清空你的所有题库, 请谨慎考虑. 如果确实要执行, 请使用 clearProblemsLib(true)")}else{
      localStorage.clear();
      console.warn("已清空题库")
    }
  }
...
Object.defineProperty(window, "stat", {
  get() {
    console.group("统计信息")
    console.group("答题方式")
    console.table({"手动":window.selectNum,"自动":window.autoSelectNum,"自动化比率":window.autoSelectNum/(window.selectNum+window.autoSelectNum)})
    console.groupEnd();
